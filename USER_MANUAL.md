...
**Before you start:** this tool applies *your* strategy and recommends. It will automatically
execute accepted signals via MetaTrader 5 if permitted. Everything it produces
...
Then confirm in MT5: **Tools → Options → Expert Advisors → Allow algorithmic trading is ON.**
The system relies on this connection to place limit orders and execute your strategy.
...
RANGE and TREND entries are limits the system can place the moment the run finishes. A SWEEP entry
depends on a candle that has not printed yet — see §5.3a, unsigned. When a setup qualifies, the engine automatically places the order in MT5.
...
## Proposed signal — Executed Automatically
...
- Volume: 0.05 lots  (partial 0.04 / runner 0.01)
...
## 4. Managing the Order
Since the system is fully automated, manual execution steps are not required. The engine places the trades and you can monitor them in the MT5 terminal.
...
4. Monitor the automated trades.
